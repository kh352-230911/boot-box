...
//    Optional<Member> findByReservation(Long id);
    ...
    @Query("SELECT m FROM Member m JOIN m.reservations r JOIN r.seats JOIN r.schedule s JOIN  s.movie JOIN s.theater t JOIN  t.cinema c JOIN  c.location WHERE m.id = :id order by r.reservationTime desc")
   Optional<Member> findByReservation(Long id);
